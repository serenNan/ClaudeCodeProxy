...
export default function CreateApiKeyForm({ onSuccess, onCancel }: CreateApiKeyFormProps) {
  ...
  const validateForm = () => {
    ...
    if (!formData.keyValue.trim()) {
      newErrors.keyValue = 'API Key不能为空';
    }
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => updateFormData('keyValue', e.target.value)}
                  placeholder="输入 API Key 值"
                  className={errors.keyValue ? 'border-red-500' : ''}
                />
                {errors.keyValue && <p className="text-red-500 text-xs">{errors.keyValue}</p>}
              </div>
            </div>
